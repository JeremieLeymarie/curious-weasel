...
const clockIn = async (date?: Date) => {
  ...
  await fetch(`${process.env.HOST}://localhost:4000/api/clocks/${userId}`, {
    method: 'POST',
...
const clockOut = async (date?: Date) => {
  ...
  await fetch(`${process.env.HOST}://localhost:4000/api/workingtimes/${userId}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  await fetch(`${process.env.HOST}://localhost:4000/api/clocks/${clock.id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
